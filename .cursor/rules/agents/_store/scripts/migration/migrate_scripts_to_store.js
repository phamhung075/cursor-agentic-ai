 ...
 * 
 * Migrates all scripts from ./scripts to .cursor/rules/agents/_store/scripts
...
class ScriptMigrationManager {
  constructor() {
    ...
    this.targetDir = '.cursor/rules/agents/_store/scripts';
    ...
    try {
      ...
      for (const [key, value] of Object.entries(packageJson.scripts)) {
        if (typeof value === 'string' && value.includes('scripts/')) {
          // Replace scripts/ with .cursor/rules/agents/_store/scripts/
          updatedScripts[key] = value.replace(/scripts\//g, '.cursor/rules/agents/_store/scripts/');
      ...
      updatedScripts['AAI:scripts-migrate'] = 'node .cursor/rules/agents/_store/scripts/migrate_scripts_to_store.js';
      updatedScripts['AAI:scripts-list'] = 'node .cursor/rules/agents/_store/scripts/script_manager.js list';
      updatedScripts['AAI:scripts-help'] = 'node .cursor/rules/agents/_store/scripts/script_manager.js help';
      updatedScripts['AAI:scripts-organize'] = 'node .cursor/rules/agents/_store/scripts/script_manager.js organize';
  ...
  async createManagementDocs() {
    ...
    const rulesPath = '.cursor/rules/agents/_store/projects/_core/rules/00__TOOLS/SCRIPT_ORGANIZATION_RULES.mdc';
    ...
    // Create script index
    const indexPath = '.cursor/rules/agents/_store/scripts/SCRIPTS_INDEX.md';
...
## 📋 IMPORTANT RULES FOR AI .cursor/rules/agents

### 🎯 **MANDATORY STORAGE LOCATIONS**

All scripts and tests created by AI .cursor/rules/agents for improvement or management MUST be stored in:

\`\`\`
.cursor/rules/agents/_store/scripts/     - All utility scripts
.cursor/rules/agents/_store/tests/       - All test files
\`\`\`

### 📂 **CATEGORIZATION SYSTEM**

#### **.cursor/rules/agents/_store/scripts/** Categories:
- \`core/\` - Core framework management
...
#### **.cursor/rules/agents/_store/tests/** Categories:
...
{
  "scripts": {
    "AAI:your-script": "node .cursor/rules/agents/_store/scripts/your_script.js"
  }
}
\`\`\`
...
When creating new utilities, AI .cursor/rules/agents MUST:

1. 🔍 **Check existing scripts** - Avoid duplication
2. 📂 **Use proper location** - \`.cursor/rules/agents/_store/scripts/\` or \`.cursor/rules/agents/_store/tests/\`
...
**Status:** ✅ **ACTIVE RULES - ALL .cursor/rules/agents MUST FOLLOW**  
...
`;
  ...
  async generateMigrationReport() {
    ...
    const reportPath = '.cursor/rules/agents/_store/logs/script_migration_report.json';
    ...
    console.log('1. Review migrated scripts in .cursor/rules/agents/_store/scripts/');
