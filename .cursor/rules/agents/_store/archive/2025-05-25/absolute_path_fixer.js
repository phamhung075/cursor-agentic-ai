  ...
  async fixFile(filePath) {
    try {
      ...
      const fixPatterns = [
        ...
        {
          pattern: /\[([^\]]+)\]\(.cursor/rules/agents\/_store\/projects\/_core\/rules\/([^)]+)\)/g,
        ...
        {
          pattern: /(?<![\[\(]).cursor/rules/agents\/_store\/projects\/_core\/rules\/([^\s\)\]\,\;]+)/g,
        ...
        {
          pattern: /\|\s*.cursor/rules/agents\/_store\/projects\/_core\/rules\/([^\s\|]+)\s*\|/g,
          fix: (match, targetPath) => {
            const relativePath = this.calculateRelativePath(dirLevels, targetPath);
            return `| ${relativePath} |`;
          },
