...
class ScriptsAnalyzer {
  ...
  analyzeScriptContent(filename, content) {
    ...
    const hasContextCore = lower.includes('_core') || lower.includes('agents/_store/projects');
      ...
      case 'update_cursor_rules_files.sh':
        if (hasContextCursor && hasRulesPath) {
          return {
            ...
            updateSuggestion: 'Update paths from .cursor/rules to agents/_store/projects/_core/rules'
          };
        }
        break;
      ...
      case 'backup_main_branch_cursor_rules.sh':
        if (hasBackup && hasContextCursor) {
          return {
            ...
            updateSuggestion: 'Update to backup agents/_store/projects/_core instead of .cursor'
          };
        }
        break;
      ...
      case 'clean_dependency_memory.js':
        if (hasDependency) {
          return {
            ...
            notes: 'Works with current agents/_store structure'
          };
        }
        break;
  ...
  async generateReport() {
    ...
    const reportPath = 'agents/_store/logs/scripts_analysis_report.json';
