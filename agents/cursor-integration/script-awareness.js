...
class CursorScriptAwareness {
  constructor() {
    this.scriptManagerPath = 'agents/_store/scripts/script_manager.js';
    ...
    for (const [category, scripts] of Object.entries(inventory.scripts)) {
      if (scripts && scripts.length > 0) {
        ...
        symbols.categorySymbols.push({
          ...
          location: `agents/_store/scripts/${category}/`
        });
      }
    }
  ...
  analyzeScriptForImprovements(script) {
    ...
    if (script.path.includes('agents/_store/scripts/') && !script.path.includes('/')) {
      improvements.push({
  ...
  async getNpmScripts() {
    try {
      ...
      return Object.entries(pkg.scripts || {})
        .filter(([name, command]) => command.includes('agents/_store/'))
